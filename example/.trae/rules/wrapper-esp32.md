...
- 对于需要注入底层wrapper对象的, 例如I2sBus, 则在构造函数中传入引用, 并在类中存储引用, 不要使用指针避免悬空引用

对于wrapper的封装可能有一些例外的情况
- 总线类的封装可能存在write和read操作, 它们的调用频率可能很高, 需要封装多个版本供上层选择
- 和原始函数保持一致的入参和返回值, 除非有特殊情况需要改变
- C++风格入参的函数, 尽可能使用不要使用指针, 使用引用
